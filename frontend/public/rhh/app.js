...
async function autorizacionesView() {
  ...
  try {
    ...
    const fetchResumen = (autTabIdx === 5) ? api('/api/rhh/nomina/vac-solicitudes/resumen') : Promise.resolve(null);

    const [vacAll, teSols, incidences, ovVales, bonoVales, vacResumen] = await Promise.all([
      ...
      isRhAdmin ? api('/api/rhh/asistencia/bonos?status=pendiente') : Promise.resolve([]),
      fetchResumen,
    ]);
    ...
    const bonoPend = bonoVales || [];
    const bonoRows = bonoPend.map(b => {
      const tipo = b.bono_type === 'limpieza' ? 'Bono Limpieza' : 'Encendido de Resistencias';
      return `<tr>
        <td><strong>${escHtml(b.employee_name || '—')}</strong></td>
        <td>${escHtml(tipo)}</td>
        <td style="text-align:center;">${escHtml(b.fecha || '—')}</td>
        <td>${escHtml(b.scheduled_shift_name || 'Sin turno')}</td>
        <td><strong>${escHtml(b.shift_worked_name || '—')}</strong></td>
        <td>${escHtml(b.solicitado_por || '—')}</td>
        <td>
          <button class="btn-primary" style="font-size:11px;padding:4px 9px;" onclick="autorizarBonoAsistencia(${b.id})">Autorizar</button>
          <button class="btn-ghost" style="font-size:11px;padding:4px 9px;color:#b91c1c;" onclick="rechazarBonoAsistencia(${b.id})">Rechazar</button>
        </td>
      </tr>`;
    }).join('');
    ...
    let vacAprobContent = '';
    if (autTabIdx === 5 && _autVacResumen && _autVacResumen.length > 0) {
    ...
    const tabs = [
      ...
      `Bonos <span class="badge" style="background:${bonoPend.length>0?'#7c3aed':'#6b7280'};font-size:10px;">${bonoPend.length}</span>`,
    ...
    if (autTabIdx === 0) {
      ...
      tabContent = isRhAdmin
        ? (bonoRows
          ? `<div style="font-size:12px;color:#5b21b6;background:#f5f3ff;border:1px solid #ddd6fe;border-radius:8px;padding:8px 12px;margin-bottom:12px;">Bonos solicitados desde Capturar Asistencia. Solo RHH/Admin puede autorizarlos.</div>
             <table><thead><tr><th>Empleado</th><th>Bono</th><th>Fecha</th><th>Turno programado</th><th>Turno trabajado</th><th>Solicita</th><th>Acción</th></tr></thead><tbody>${bonoRows}</tbody></table>`
          : '<div class="empty-state"><div class="empty-icon">✅</div><p>Sin bonos pendientes</p></div>')
        : '<div class="empty-state"><p>La autorización de bonos corresponde a RHH/Admin.</p></div>';
    } else {
...
async function autEditVacDias(solId) {
  const ini = document.getElementById(`aut-vac-ini-${solId}`)?.value;
  const fin = document.getElementById(`aut-vac-fin-${solId}`)?.value;
  ...
  try {
    const r = await api(`/api/rhh/nomina/vac-solicitudes/${solId}/editar-dias`, { method: 'PATCH', body: JSON.stringify({ fecha_inicio: ini, fecha_fin: fin }) });
    toast(`Fechas actualizadas (${r.dias || '?'} dias efectivos)`);
    autorizacionesView();
  } catch (err) { toast(err.message, 'error'); }
}

async function autResolverCambioVac(solId, accion) {
  let respuesta = '';
  if (accion === 'rechazar') {
    respuesta = prompt('Escribe la razon del rechazo para el empleado:');
    if (respuesta === null) return;
  } else {
    if (!confirm('Aprobar este cambio/cancelacion?')) return;
  }
  try {
    await api(`/api/rhh/nomina/vac-solicitudes/${solId}/resolver-cambio`, { method: 'PATCH', body: JSON.stringify({ accion, respuesta }) });
...
async function autorizarBonoAsistencia(id) {
  if (!confirm('¿Autorizar este bono?')) return;
  try {
    await api(`/api/rhh/asistencia/bonos/${id}/autorizar`, { method: 'POST' });
    toast('Bono autorizado');
    autorizacionesView();
  } catch (err) { toast(err.message, 'error'); }
}

async function rechazarBonoAsistencia(id) {
  const notas = prompt('Motivo del rechazo (opcional):');
  if (notas === null) return;
  try {
    await api(`/api/rhh/asistencia/bonos/${id}/rechazar`, {
      method: 'POST',
      body: JSON.stringify({ notas_rechazo: notas })
    });
    toast('Bono rechazado');
    autorizacionesView();
  } catch (err) { toast(err.message, 'error'); }
}
...
async function revertirBajaPlantilla(empId, nombre, btnEl) {
  if (!confirm(`¿Revertir la baja de ${nombre}?\n\nEl empleado volverá a aparecer en Incidencias, Lista de Raya y Rol Semanal.`)) return;
  try {
    await api(`/api/rhh/catalogo/${empId}/info`, {
      method: 'PATCH',
      body: JSON.stringify({ status: 'active', fecha_baja: null })
    });
    btnEl.textContent = '✓ Revertida';
    btnEl.disabled = true;
    btnEl.style.color = '#059669';
    toast(`Baja revertida: ${nombre}`, 'success');
  } catch (err) {
...
async function asisCaptureView() {
  ...
  try {
    ...
    const bonosWeek       = data.bonos_week      || [];
    const today           = data.today           || '';
    const role            = state.user?.role;
    const isRHHAdmin      = ['rh','admin'].includes(role);
    const canCaptureBonos = ['supervisor','rh','admin'].includes(role);
    const canParoTecnico  = isRHHAdmin;
    // /diaria incluye las horas reales de cada turno; reutilizarlas en el modal TXT.
    state.shifts = shifts;
    ...
    if (selFecha) {
      ...
      for (const group of shiftGroups) {
        ...
        let rowBtn = '';
        if (specialConflict) {
          rowBtn = `<button disabled style="padding:4px 10px;border:none;border-radius:6px;background:#fee2e2;color:#991b1b;font-size:10px;cursor:not-allowed;">No puede laborar</button>`;
        ...
        const disTE_final = specialConflict || (isRestDay ? (!teActivo || teBlockedByTxt) : (!editable || !teActivo || teBlockedByTxt));

        // Cumpleaños
        const isBirthday = !!dayData.is_birthday;
        // Bonos del día
        const dayBonos = dayData.bonos || [];
        const empBonosWeek = bonosWeek.filter(b => Number(b.employee_id) === Number(emp.employee_id));
        const resistenciaWeek = bonosWeek.find(b => b.bono_type === 'encendido_resistencias');

        // Turno incompleto: horas pendientes
        const hpTurno = dayData.horas_pendientes_turno || '';
        const isIncTurnoInc = inc === 'turno_incompleto';

        // Bono: sábado/domingo. El turno realmente trabajado se valida al solicitar.
        ...
        rowsHtml += `<tr style="${rowStyle}">
          ...
          <td style="padding:4px 6px;text-align:center;border-left:2px solid #fed7aa;">
            ${teBlockedByTxt
              ? `<span style="display:inline-block;font-size:9px;color:#dc2626;font-weight:700;max-width:130px;line-height:1.2;" title="No puede hacer TE: debe ${txtHorasPend}h">No puede registrar TE: debe ${txtHorasPend} h TXT</span>`
              : specialConflict
                ? '<span style="font-size:9px;color:#991b1b;font-weight:700;">No puede laborar</span>'
            ...
            <div style="display:flex;flex-direction:column;gap:3px;align-items:center;">
              ...
              ${canBono && canCaptureBonos && !specialConflict ? `<div style="display:flex;gap:3px;margin-top:2px;">
                ${['limpieza','encendido_resistencias'].map(bt => {
                  ...
                  if (bt === 'encendido_resistencias' && resistenciaWeek) {
                    return `<span title="Ya fue asignado a otro trabajador esta semana" style="font-size:9px;padding:2px 5px;border-radius:4px;background:#f1f5f9;color:#64748b;font-weight:600;">B.Enc.Res: asignado</span>`;
                  }
    ...
    const content = `
      ...
      ${selFecha && new Date(selFecha+'T12:00:00').getDay()===0 ? '<div style="font-size:12px;color:#ea580c;background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:8px 12px;margin-bottom:10px;font-weight:600;">Domingo — día de descanso. Se puede registrar TE, pago TXT y bonos permitidos.</div>' : ''}
...
async function asisTxtPagarConfirm(deudaId, empId, fecha, attendanceId) {
  ...
  if (modal) modal.remove();
  if (r) {
    const msg = r.horas_sobrante > 0
      ? `TXT registrado. ${r.horas_sobrante} h generaron un vale de TE pendiente de autorización.`
...
async function asisCrearBono(empId, fecha, bonoType, attendanceId) {
  ...
  const shifts = state.shifts || [];
  if (!shifts.length) { toast('No se encontró el catálogo de turnos', 'error'); return; }
  const shiftOpts = shifts.map(s => `<option value="${s.id}">${escHtml(s.name)} (${s.hours || '?'} h)</option>`).join('');
  const html = `<div style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:grid;place-items:center;" id="bono-asistencia-modal">
    <div style="background:#fff;border-radius:14px;width:min(420px,94vw);padding:18px;box-shadow:0 20px 60px rgba(0,0,0,.25);">
      <div style="font-size:16px;font-weight:800;margin-bottom:4px;">Solicitar ${lbl}</div>
      <div style="font-size:12px;color:#64748b;margin-bottom:14px;">${fecha} · confirma el turno realmente trabajado. Esto no cambia el Rol Semanal.</div>
      <label style="font-size:12px;font-weight:700;">Turno realmente trabajado</label>
      <select id="bono-shift-worked" style="width:100%;padding:8px;margin-top:4px;border:1px solid #cbd5e1;border-radius:7px;">${shiftOpts}</select>
      <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:16px;">
        <button class="btn-ghost" onclick="document.getElementById('bono-asistencia-modal').remove()">Cancelar</button>
        <button class="btn-primary" onclick="asisConfirmarBono(${empId},'${fecha}','${bonoType}',${attendanceId||'null'})">Enviar a autorización</button>
      </div>
    </div>
  </div>`;
  document.body.insertAdjacentHTML('beforeend', html);
...
async function asisConfirmarBono(empId, fecha, bonoType, attendanceId) {
  const shiftWorkedId = Number(document.getElementById('bono-shift-worked')?.value);
  if (!shiftWorkedId) { toast('Selecciona el turno realmente trabajado', 'warning'); return; }
  const lbl = bonoType === 'limpieza' ? 'Bono Limpieza' : 'Bono Enc. Resistencias';
  const r = await api('/api/rhh/asistencia/bonos', {
    method: 'POST',
    body: JSON.stringify({ employee_id: empId, fecha, bono_type: bonoType, attendance_id: attendanceId, shift_worked_id: shiftWorkedId })
  });
  if (r) {
    document.getElementById('bono-asistencia-modal')?.remove();
    toast(`${lbl} enviado a autorización`, 'success');
    asisCaptureView();
  }
}
  ...
  try {
    ...
    for (const emp of grid) {
      ...
      const comentarios = (emp.comentarios || []).map(c => {
        ...
        if (c.type === 'txt_pago') color = '#047857';
        if (c.type === 'cumpleanos') color = '#ec4899';
        if (c.type === 'festivo') color = '#c2410c';
        if (c.type === 'especial_no_labora') color = '#991b1b';
...
async function kpiRhhView() {
  ...
  function buildCostosProyTable(data, period) {
    ...
    if (!departments.length) return '<div class="empty-state"><p>Sin datos de departamentos</p></div>';

    const cols = 2 + labels.length;
    const thCols = labels.map(l => `<th style="text-align:right;padding:6px 8px;white-space:nowrap;background:${TH_BG};color:#111;font-size:11px">${escHtml(l)}</th>`).join('');
    const periodTotals = labels.map((_, i) => departments.reduce((sum, d) => sum + ((d[byKey] || [])[i]?.amount || 0), 0));
    const grandTotal = periodTotals.reduce((s, t) => s + t, 0);

    const rows = departments.map((dept, idx) => {
      const byP = dept[byKey] || [];
      const rowTotal = byP.reduce((s, p) => s + (p.amount || 0), 0);
      const bg = idx % 2 === 0 ? '#fff' : '#f8fafc';
      return `<tr style="border-top:1px solid #e5e7eb">
        <td style="padding:7px 12px;font-weight:600;color:#111;position:sticky;left:0;background:${bg};z-index:1">${escHtml(dept.name)} <span style="color:#9ca3af;font-weight:400;font-size:11px">(${dept.employees} emps)</span></td>
    ...
    let html = `
      <div class="table-wrap" style="overflow-x:auto;-webkit-overflow-scrolling:touch">
        <table style="font-size:12px;border-collapse:collapse;min-width:max-content;width:100%">
          <thead><tr>
            <th style="text-align:left;padding:6px 12px;background:${TH_BG};color:#111;min-width:220px;position:sticky;left:0;z-index:2">Departamento</th>
    ...
    html += `
      <div style="margin-top:28px;padding-top:20px;border-top:2px solid #e5e7eb">
        <h4 style="margin:0 0 14px;font-size:14px;color:#1e3a8a">📊 Pareto de Gastos por Departamento</h4>
    ...
    sorted.forEach((dept, idx) => {
      ...
      if (puestos.length > 0) {
        const pRows = puestos.map(p => {
          ...
          return `<div style="display:flex;align-items:center;gap:8px;padding:3px 0;font-size:11px;color:#475569">
            <span style="flex:0 0 180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${escHtml(p.name)}">${escHtml(p.name)}</span>
            <span style="flex:0 0 55px;text-align:right;font-weight:600;color:#111">${fmt$(p.total)}</span>
            <span style="flex:0 0 40px;text-align:right">${ppct.toFixed(1)}%</span>
            <div style="flex:1;height:8px;background:#e5e7eb;border-radius:4px;overflow:hidden">
              <div style="width:${Math.min(ppct, 100)}%;height:100%;background:${color};border-radius:4px;opacity:.7"></div>
            </div>
      ...
      html += `
        <div style="background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:10px 14px">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="flex:0 0 170px;font-weight:700;font-size:13px;color:#111;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${escHtml(dept.name)}">${escHtml(dept.name)}</div>
            <div style="flex:0 0 90px;text-align:right;font-weight:700;font-size:13px;color:#111">${fmt$(dept.total)}</div>
            <div style="flex:0 0 50px;text-align:right;font-size:12px;color:#475569">${pct.toFixed(1)}%</div>
            <div style="flex:1;height:14px;background:#e5e7eb;border-radius:6px;overflow:hidden;position:relative">
  ...
  try {
    ...
    async function reloadProData(semDesde, semHasta, deptId) {
      ...
      proyData = await api('/api/rhh/kpi/costos-proyecto' + (qs ? '?' + qs : ''));
      renderPro();
    }
    function renderInc() {
    ...
    el.innerHTML = shell(`
      <div style="display:flex;gap:8px;margin-bottom:20px;border-bottom:2px solid #e5e7eb;padding-bottom:0;flex-wrap:wrap">
        ...
        <button id="kpi-tab-pro" class="tab-btn"            style="padding:10px 20px;font-size:13px;font-weight:700">🏗️ Costos por Departamento</button>
      ...
      <!-- Panel Costos por Departamento -->
      <div id="kpi-panel-pro" style="display:none">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px">
          <h4 style="margin:0;font-size:15px;color:#1d4ed8">🏗️ Costos por Departamento</h4>
          <div style="display:flex;gap:4px">
        ...
        <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:12px;padding:10px 14px;background:#f1f5f9;border-radius:8px">
          ...
          <select id="kpi-pro-sem-desde" style="padding:4px 8px;font-size:12px;border:1px solid #cbd5e1;border-radius:4px"></select>
          <label style="font-size:12px;font-weight:600;color:#334155">hasta:</label>
          <select id="kpi-pro-sem-hasta" style="padding:4px 8px;font-size:12px;border:1px solid #cbd5e1;border-radius:4px"></select>
          <label style="font-size:12px;font-weight:600;color:#334155;margin-left:8px">Departamento:</label>
          <select id="kpi-pro-dept" style="padding:4px 8px;font-size:12px;border:1px solid #cbd5e1;border-radius:4px;max-width:200px"></select>
          <button id="kpi-pro-filter-btn" class="btn-primary" style="padding:4px 14px;font-size:12px">Filtrar</button>
          <button id="kpi-pro-reset-btn" class="btn-secondary" style="padding:4px 14px;font-size:12px">Limpiar</button>
        </div>
        <p class="small muted" style="margin:0 0 12px">Costo total por departamento (Nomina + TE + Bonos + Vacaciones + Despensa) con desglose por puesto.</p>
    ...
    // Poblar filtros de costos por departamento
    const allWeeks = proyData.all_weeks || [];
    const allDepts = proyData.all_depts || [];
    const selDesde = document.getElementById('kpi-pro-sem-desde');
    const selHasta = document.getElementById('kpi-pro-sem-hasta');
    const selDept  = document.getElementById('kpi-pro-dept');
    selDesde.innerHTML = '<option value="">Todas</option>' + allWeeks.map(w => `<option value="${w.no_periodo}">${w.label}</option>`).join('');
    selHasta.innerHTML = '<option value="">Todas</option>' + allWeeks.map(w => `<option value="${w.no_periodo}">${w.label}</option>`).join('');
    selDept.innerHTML  = '<option value="">Todos</option>' + allDepts.map(d => `<option value="${d.id}">${escHtml(d.name)}</option>`).join('');
    document.getElementById('kpi-pro-filter-btn').onclick = () => {
      reloadProData(selDesde.value, selHasta.value, selDept.value);
    };
    document.getElementById('kpi-pro-reset-btn').onclick = () => {
      selDesde.value = ''; selHasta.value = ''; selDept.value = '';
      reloadProData(null, null, null);
    };
    ...
    <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:12px;padding:14px 18px;margin-bottom:16px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:12px;">
        ...
        <div style="text-align:center;background:#fff;border-radius:8px;padding:10px;">
          ...
          <div style="font-size:10px;color:#64748b;">(solicitudes pend.)</div>
        </div>
